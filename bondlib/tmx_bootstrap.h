...
namespace tmx::bootstrap {
	...
	constexpr std::pair<T, F> instrument(const tmx::instrument<U,C>& i, const tmx::curve<T,F> f, 
		F p = 0, F _f = NaN<F>)
	{
		ensure (i.size() != 0);
		...
		if (f.size() == 0) {
			...
			_f = secant::solve([&](F f0) {
				return value::present(i, curve_constant(f0)) - p; }, _f, _f + 0.01);
		}
		else {
	...
	/*
	template<class U = double, class C = double, class T = double, class F = double>
	constexpr curve::curve_value<T, F> instruments(iterable<instrument<U,C>> is)
	{
		curve::curve_value<T, F> f;
